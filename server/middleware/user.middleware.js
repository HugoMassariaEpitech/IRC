...
generateNameSalt = async (req, res, next) => {
  ...
  User.find({"identification.name": req.body.identification.name}).then((result) => {
    req.body.identification.nameSalt = findTheNextNameSalt(result);
    next();
  }).catch((error) => {
...
findTheNextNameSalt = (users) => {
  if (users.length > 0) {
    ...
    for (let i = 0; i < users.length; i++) {
      ...
      if (users[i].identification.nameSalt > max) {
        max = users[i].identification.nameSalt;
      }
    }
    return (max + 1);
  } else {
